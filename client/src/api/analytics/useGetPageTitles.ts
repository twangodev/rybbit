...
// Hook for StandardSection-like usage (fetches a limited, non-paginated list)
export function useGetPageTitlesForStandardSection({
  limit = 10, // StandardSection typically shows a small list
  useFilters = true,
}: Pick<UseGetPageTitlesOptions, "limit" | "useFilters"> = {}): UseQueryResult<
  APIResponse<PageTitlesStandardResponse> // Expects direct array
> {
  const { time, site, filters } = useStore();
