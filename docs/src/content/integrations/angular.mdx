...
The most common and recommended method is to add the Rybbit tracking script directly to your `index.html` file.
...
### (Advanced) Dynamically Loading the Script via a Service

For more complex scenarios or if you need to conditionally load the script, you can create an Angular service.

```typescript filename="src/app/rybbit-tracking.service.ts"
...
export class RybbitTrackingService {{
  private RYBBIT_SITE_ID = 'YOUR_SITE_ID'; // Replace with your Site ID

  constructor(@Inject(PLATFORM_ID) private platformId: Object) {{}}

  loadScript(): void {{
    if (isPlatformBrowser(this.platformId)) {{ // Ensure running in browser
      const existingScript = document.querySelector(`script[data-site-id="${{this.RYBBIT_SITE_ID}}"]`);

      if (!existingScript) {{
        const script = document.createElement('script');
        script.src = 'https://app.rybbit.io/api/script.js';
        script.async = true;
        script.setAttribute('data-site-id', this.RYBBIT_SITE_ID);
...
```
You would then inject this service into your root component (`AppComponent`) or another appropriate place and call `loadScript()`:

```typescript filename="src/app/app.component.ts"
import { Component, OnInit } from '@angular/core';
import { RybbitTrackingService } from './rybbit-tracking.service';
...
export class AppComponent implements OnInit {{
  ...
  constructor(private rybbitTrackingService: RybbitTrackingService) {{}}

  ngOnInit(): void {{
    this.rybbitTrackingService.loadScript();
  }}
}}
```
