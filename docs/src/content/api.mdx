...
### Optional
- `type` - Event type: `pageview` or `custom_event` (defaults to `pageview`)
...
- `user_agent` - Custom user agent string (max 512 chars) - will be parsed for browser/OS/device info
- `ip_address` - Custom IP for geolocation - will be resolved to location data
...
class RybbitTracker {
    ...
    private let apiKey = ProcessInfo.processInfo.environment["RYBBIT_API_KEY"] ?? ""
    private let siteId = ProcessInfo.processInfo.environment["RYBBIT_SITE_ID"] ?? ""
    ...
    func trackEvent(_ eventData: [String: Any]) async throws {
        ...
        data["screenWidth"] = Int(UIScreen.main.bounds.width)
        data["screenHeight"] = Int(UIScreen.main.bounds.height)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            tracker.trackEvent(mapOf(
                ...
                "pathname" to "/shop",
                "event_name" to "add_to_cart",
                "properties" to """{"product_id": "123", "price": 29.99}"""
            ))
        }
    }
}
...
class RybbitTracker {
  ...
  Future<void> trackEvent(Map<String, dynamic> eventData) async {
    ...
    data['screenWidth'] = WidgetsBinding.instance.window.physicalSize.width.toInt();
    data['screenHeight'] = WidgetsBinding.instance.window.physicalSize.height.toInt();
          ...
          child: ElevatedButton(
            onPressed: () {
              ...
              tracker.trackEvent({
                ...
                'properties': json.encode({'button_id': 'main_cta'}),
              });
            },
