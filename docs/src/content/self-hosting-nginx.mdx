...
server {
    ...
    location / {
        proxy_pass http://localhost:3002;  # Client port handles all requests
...
<Callout type="info">
  With the new architecture, you only need to proxy all requests to the Next.js client port (3002). The client handles API routing internally, eliminating the need for complex reverse proxy configurations.
</Callout>
