...
#### Option A: With Built-in Caddy (Automatic SSL)
...
#### Option B: Without Caddy (Custom Reverse Proxy)
...
#### Traefik Labels
Add these labels to your services in docker-compose.override.yml:
```yaml
services:
  backend:
    labels:
...
- **rybbit-client**: Next.js frontend (port 3002)
- **rybbit-backend**: Node.js API server (port 3001)
- **rybbit-postgres**: User data and configuration
- **rybbit-clickhouse**: Analytics data storage
- **rybbit-caddy**: Reverse proxy with automatic SSL (optional)
...
<Callout type="info">
For more advanced configurations and troubleshooting, see our [Advanced Self-Hosting Guide](/docs/self-hosting-advanced) and [Nginx Setup Guide](/docs/self-hosting-nginx).
</Callout>